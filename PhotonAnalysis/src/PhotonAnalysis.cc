...
bool PhotonAnalysis::VBFTag2013(int & ijet1, int & ijet2, LoopAll& l, int& diphotonVBF_id, float* smeared_pho_energy, 
    ...
    if(mvaselection){
        ...
        ComputeCombinedDijetCategory2013(l, vbfcat, mvaselection);
...
void PhotonAnalysis::ComputeCombinedDijetCategory2013(LoopAll& l, int& vbfcat, bool mvaselection) {
    vbfcat=-1;
    if(mvaselection){
        for(int ivbfcat=0; ivbfcat<multiclassVbfCatBoundaries0.size(); ivbfcat++){
            if( myVBFDIPHObdt > multiclassVbfCatBoundaries0[ivbfcat] && myVBFcombined > multiclassVbfCatBoundaries1[ivbfcat]){
                if(PADEBUG) std::cout<<"cut dijetmva combinedmva "<<ivbfcat<<" "<<multiclassVbfCatBoundaries0[ivbfcat]<<" "<<multiclassVbfCatBoundaries1[ivbfcat]<<std::endl;
                vbfcat++;
            }
        }
        if(PADEBUG) std::cout<<"vbfcat dijetmva combinedmva "<<vbfcat<<" "<<myVBFDIPHObdt<<" "<<myVBFcombined<<std::endl;
    }
}
